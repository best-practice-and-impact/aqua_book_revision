...
**Verification and validation:** Analytical quality assurance is more than checking that the analysis is error-free and satisfies its specification (validation). It should also include checks that the analysis is appropriate, i.e. fit for the purpose for which it is being used (verification). Validation and verification are covered in more depth in chapters [5-9]. 
